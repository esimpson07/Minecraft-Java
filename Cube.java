...
public class Cube {
    ...
    private double[] angle;import java.awt.Color;
...
public class Cube {
                ...
                if(adjacentChunks[i] != -1) {
                    for(int j = 0; j < Screen.Chunks[adjacentChunks[i]].getCubeArray().size(); j ++) {
                        if((Screen.Chunks[adjacentChunks[i]].getCubeArray().get(j).getCoords()[0] == getAdjacentCube(f)[0] && Screen.Chunks[adjacentChunks[i]].getCubeArray().
                            ...
                            polysToDraw[f] = false;
                            updatePoly();
                            Screen.Chunks[adjacentChunks[i]].getCubeArray().get(j).softAdjacencyCheck();
                        }
                    }
                }
            }
    ...
    static void hardAdjacencyCheck(double[][] adjacentCubes) {
        int chunk = Screen.getChunkNumberIn((int)adjacentCubes[6][0],(int)adjacentCubes[6][1]);
        int sideLength = Screen.worldSize / Screen.chunkSize;
        int[] adjacentChunks = new int[5]; //left (chunk - 1) right (chunk + 1) above (chunk + sideLength) below (chunk - sideLength)
        if(chunk % sideLength != 0 && (int)adjacentCubes[6][0] % Screen.chunkSize == 0) {
            adjacentChunks[0] = chunk - 1;
        } else {
            adjacentChunks[0] = -1;
        }
        if(chunk % sideLength != sideLength - 1 && (int)adjacentCubes[6][0] % Screen.chunkSize == Screen.chunkSize - 1) {
            adjacentChunks[1] = chunk + 1;
        } else {
            adjacentChunks[1] = -1;
        }
        if(chunk >= sideLength && (int)adjacentCubes[6][1] % Screen.chunkSize == 0) {
            adjacentChunks[2] = chunk - sideLength;
        } else {
            adjacentChunks[2] = -1;
        }
        if(chunk < Math.pow(sideLength,2) - sideLength && (int)adjacentCubes[6][1] % Screen.chunkSize == Screen.chunkSize - 1) {
            adjacentChunks[3] = chunk + sideLength;
        } else {
            adjacentChunks[3] = -1;
        }
